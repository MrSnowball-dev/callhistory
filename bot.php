...
if ($message == '/start') {
	...
	if ($sql_chat_id == $chat_id) {
		...
		mysqli_query($db, "insert into users (chat_id, acr_secret) values (".$chat_id.", aes_encrypt('".$acr_secret."', 'ACR'))");
		sleep(5);
...
if ($message == '/secret') {
	$query = mysqli_query($db, "select aes_decrypt('acr_secret', 'ACR') as 'acr_secret' from users where chat_id=".$chat_id);
	while ($sql = mysqli_fetch_object($query)) {
